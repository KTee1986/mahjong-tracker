  ...
  useEffect(() => {
    fetch("/api/sheet")
      ...
      .then(({ data }) => {
        ...
        const mostPlayed = Object.entries(playerStats).sort(([, a], [, b]) => b.played - a.played)[0][0];
        const leastPlayed = Object.entries(playerStats).sort(([, a], [, b]) => a.played - b.played)[0][0];
        const biggestWin = Object.entries(playerStats).sort(([, a], [, b]) => b.highestWin.score - a.highestWin.score)[0];
        const biggestLoss = Object.entries(playerStats).sort(([, a], [, b]) => a.highestLoss.score - b.highestLoss.score)[0];
