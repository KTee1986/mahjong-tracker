...
/**
 * Fetches permissions for a specific Settle Up group via REST API.
 * @param {string} groupId - The ID of the group.
 * @param {string} token - The Firebase auth token.
 * @returns {Promise<object|null>} - Resolves with an object where keys are UIDs and values are { level: number }, or null on error.
 */
export async function getGroupPermissions(groupId, token) {
    if (!firebaseConfig.databaseURL) throw new Error("Firebase databaseURL is not configured.");
    const url = `${firebaseConfig.databaseURL}/permissions/${groupId}.json?auth=${token}`;
    console.log("Fetching permissions via REST for group:", groupId);
    try {
        const response = await axios.get(url);
        // console.log(`Successfully fetched permissions structure via REST for group ${groupId}:`, response.data ? Object.keys(response.data) : 'None');
        return response.data; // e.g., { "uid1": { level: 30 }, ... } or null
    } catch (error) {
        const status = error.response?.status;
        console.error(`Error fetching permissions via REST for group ${groupId}:`, status, error.response?.data || error.message);
        if (status === 401 || status === 403) {
            throw new Error(`Authentication error fetching permissions for group ${groupId}.`);
        }
        if (status === 404) {
             console.warn(`Permissions path not found via REST for group ID ${groupId}.`);
             return null; // Group might exist but have no permissions path yet
        }
        // For other errors, return null
        console.error(`Non-auth/404 error fetching permissions via REST for group ${groupId}.`);
        return null;
    }
}
